...
const StripePaymentForm: React.FC<{
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      onError(err instanceof Error ? err.message : 'An error occurred during payment');
    }
  };
...
const CheckoutPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setCartItems(sampleCartItems.filter(item => item.checked));
  }, []);
  ...
  const handleCheckout = async () => {
    ...
    try {
      switch(paymentMethod) {
        case 'stripe': {
          const productResponse = await paymentService.createProduct({
            title: `Order from ${cartItems.map(item => item.title).join(', ')}`,
          ...
          const paymentResponse = await paymentService.createPaymentIntent({
            ...
            currency: 'usd',
            paymentMethodId: '',
            productId: productResponse.stripeProductId,
        ...
        <CheckoutContainer>
          ...
          <SidePanel>
            <OrderSummarySection>
              ...
              >
                {loading ? 'Processing...' : getPaymentButtonText()}
              </CheckoutButton>
              
              {error && (
                <div style={{ 
                  color: huskyTheme.colors.error,
                  marginTop: '1rem',
                  textAlign: 'center'
                }}>
                  {error}
              ...
              <LegalSection>
                By placing your order, you agree to UniMart's{' '}
                <LegalLink href="/user-agreement">User Agreement</LegalLink> and{' '}
                <LegalLink href="/privacy-notice">Privacy Notice</LegalLink>.
              </LegalSection>
            </OrderSummarySection>
          </SidePanel>
        </CheckoutContainer>
